...
public class Request {
	...
	public Response postRequest(String endPoint, String body) {
		return given().filter(new RequestLoggingFilter(getRequestLogger())).headers(headers.getHeaders()).body(body)
				.post(endPoint);
	}
	 ...
	 * @return
	 */
	public Response getRequest(String endPoint, HashMap<String, ?> maps) {
		if (maps != null) {
			return given().when().queryParams(maps).filter(new RequestLoggingFilter(getRequestLogger()))
					.headers(headers.getHeaders()).get(endPoint);
		} else
			return given().filter(new RequestLoggingFilter(getRequestLogger())).headers(headers.getHeaders())
					.get(endPoint);
	}
	...
	public Response deleteRequest(String endPoint) {
		return given().filter(new RequestLoggingFilter(getRequestLogger())).headers(headers.getHeaders()).body("test")
				.delete(endPoint);
	}
}
